...
export class FriendsComponent {
  ...
  constructor(private http: HttpClient, private router: Router) {
    ...
    const followingId=localStorage.getItem('followingId');
    ...
    this.http.get<any[]>('http://localhost:5100/follow').subscribe((res: any[]) => {
      this.followingData = res.filter((follow) => follow.followingId._id === userId);
    });
    ...
    this.http.get<any[]>(`http://localhost:5100/follow`).subscribe((res)=>{
      this.following = res.filter((follow) => follow.userId === userId);
      this.following = res
    })
